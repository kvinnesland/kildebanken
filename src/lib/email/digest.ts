 ...
 * bygget ennå — fargeverdiene under er skrevet direkte som de faktiske
 * verdiene fra tokens/primitives.css og tokens/semantic.css i dag. Endres
 * designtokenene, må disse oppdateres manuelt inntil den pipelinen finnes.
 * Se NATTLOGG.md.
 */
    ...
    .map((r) => {
      ...
      const languageNotice =
        r.contentLanguage !== locale
          ? `<p style="color:#6b6f76;font-size:13px;margin:4px 0 0;">${escapeHtml(
              t("request.foreign_language_notice")
            )}</p>`
          : "";
      const geoLine = r.geographicNote
        ? `<p style="color:#6b6f76;font-size:14px;margin:4px 0 0;">${escapeHtml(r.geographicNote)}</p>`
        : "";

      return `<tr><td style="padding:16px 0;border-bottom:1px solid #e3e6ea;">
        <h2 style="font-size:18px;margin:0 0 8px;font-family:Georgia,'Times New Roman',serif;">
          <a href="${url}" style="color:#1d5b91;text-decoration:none;">${escapeHtml(r.title)}</a>
        </h2>
        <p style="margin:0 0 8px;color:#21242b;">${escapeHtml(r.summary)}</p>
        <p style="color:#6b6f76;font-size:13px;margin:0;">${escapeHtml(r.organizationName)}</p>
        <p style="color:#6b6f76;font-size:13px;margin:2px 0 0;">${escapeHtml(
          t("request.deadline_label", { deadline: dateFormatter.format(r.responseDeadline) })
        )}</p>
        ${geoLine}${languageNotice}
        <a href="${url}" style="display:inline-block;margin-top:10px;padding:8px 16px;background:#1d5b91;color:#ffffff;text-decoration:none;border-radius:6px;font-size:14px;">${escapeHtml(
    ...
    <tr><td align="center" style="padding:24px 12px;">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:8px;">
        <tr><td style="padding:24px;">
          <p style="margin:0 0 20px;color:#21242b;">${escapeHtml(t("digest.intro"))}</p>
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">${itemsHtml}</table>
          <p style="margin:24px 0 0;font-size:12px;color:#6b6f76;">
            <a href="${unsubscribeUrl}" style="color:#6b6f76;">${escapeHtml(t("digest.unsubscribe"))}</a>
          </p>
        </td></tr>
      </table>
    </td></tr>
