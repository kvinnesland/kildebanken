...
import { z } from "zod";
import { suspendUser } from "@/lib/moderation/users";

const bodySchema = z.object({ reason: z.string().min(1).max(2000) });

// POST /admin/users/:id/suspend (SPEC-V1.md 20, 8.1, 16.2). Begrunnelse er
// obligatorisk — se suspendUser() i src/lib/moderation/users.ts.
